...
const CONFIG_FIELDS = {
  app: [['username_change_days','Username cooldown (days)'],['old_username_reserve_days','Old username reserve'],['min_interests','Min interests']],
...
function initials(name='CodaVybes') { return String(name ?? '').trim().split(/\s+/).slice(0,2).map(v=>v[0]?.toUpperCase()).join('') || 'V' }
...
function PromotionsPanel({ rows, adsEnabled, canEdit, onRefresh, notify }) {
  const empty={id:null,brand_name:'',headline:'',body:'',image_url:'',destination_url:'',cta_label:'Learn more',active:true,priority:0,starts_at:'',ends_at:''}
  ...
  return <div className="hq-view">
    ...
    <div className="hq-grid-2">
      <article className="hq-card">
        ...
        <p className="hq-help">Short feeds show one sponsored card after the first organic post. Larger feeds use a low-frequency placement after roughly every seven organic posts, capped at two sponsored cards per feed window. Campaign start and end settings still apply. Sponsored delivery is not segmented by age.</p>
      </article>
      <article className="hq-card">
        ...
        <div className="hq-promo-list">{rows.length?rows.map(r=>{
          ...
          return <button key={r.id} className={`hq-promo-row ${state==='LIVE'?'':'is-off'}`} onClick={()=>edit(r)}>
            ...
            <section><strong>{r.brand_name}</strong><b>{r.headline}</b><small>{Number(r.impressions||0).toLocaleString()} impressions · {Number(r.clicks||0).toLocaleString()} clicks</small></section>
            <em>{state}</em>
          </button>
